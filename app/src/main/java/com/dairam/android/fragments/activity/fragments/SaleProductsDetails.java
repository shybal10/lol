...
public class SaleProductsDetails extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		cartItButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (id == null) {
					...
					bundle.putString("Page", "saledetails");
					bundle.putString("Bannertitle", banerTitle);
		 ...
		 * ((Integer.parseInt(quantity) < 10)) { Log.e("entered into here",
		 * Integer.parseInt(quantity) + ""); for (int i = 1; i <=
		 * Integer.parseInt(quantity); i++) { String dummy =
		 * Integer.toString(i); list.add(dummy); } } else { list.add("1");
		...
		try {
			...
			backToPreviousUrl = getArguments().getString("referenceURl");
			banerTitle = getArguments().getString("BannerTitle");
	...
	private class DownloadJSON extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			jsonobject = JSONfunctions.getJSONfromURL(PRODUCTDETAILURL);

			Log.e("The product detailsa", PRODUCTDETAILURL);
			...
			try {
				...
				if (jsonobject != null) {
					if (jsonobject.getBoolean("success")) {
						...
						imageString = jsonobject.getString("image2");

						if (imageString != null) {
							imageIdList.add(imageString);
						}
						imageString = jsonobject.getString("image3");
						...
						Log.e("SaleProdDetails", "Maximum Stock : " + maxStock);
						if (maxStock > 10) {
							maxStock = 10;
						}
						if (maxStock == 1) {
