...
public class PromoItemSingleProductDetailsFragment extends Fragment {
	...
	double productPriceValue = 0.000;
	...
	/*
	 * Button leftarrowButton, rightArrowButton, leftarrowButton1,
	 * rightArrowButton1;
	 */
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		// v.findViewById(R.id.circle_indicator);
			...
			public void onClick(View v) {
				...
				if (id == null) {
					...
					addToCartURL = addToCartURL + product_id + "&id=" + id
							+ "&quantity="
							+ quantityText.getText().toString().trim()
							+ "&language=1";
		 ...
		 * list.add("9"); list.add("10"); } ArrayAdapter<String> dataAdapter =
		 * new ArrayAdapter<String>( getActivity(), R.layout.spinneritems,
		 * list);
		...
		shareButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// ShareDet();
				/*
				 * Log.e("Share Button","In PromoItemSingleDetailsFragment");
				 * intShare();
				 */
				Log.e("Share Button", "In PromoItemSingleDetailsFragment");
...
//				MyDialogFragmentSocialmediaShare myDialogFragmentsocial = new MyDialogFragmentSocialmediaShare(
//						shareProdName, shareProdDescription, shareImageString,
//						tempurl);
//
...
//						"myDialogFragment Share");
		...
		try {
			...
			Log.e("PromoSingleItem", "Prod Id: " + productid);
				...
				if (jsonobject != null) {
					if (jsonobject.getBoolean("success")) {
						...
						if (getActivity() != null) {
							SharedPreferences sp1 = getActivity()
									.getSharedPreferences("tag",
											Context.MODE_PRIVATE);
							sp1.edit().putString("productid", productid)
									.commit();
						}
						...
						try {
							...
							/*
							 * ArrayAdapter<String>descadapter=new
							 * ArrayAdapter<>(context,
							 * android.R.layout.simple_list_item_1, desc);
							 * descriptionTextView.setAdapter(descadapter);
							 */
						...
						if ((CurrencyCode.equals("KD"))
							...
							try {
								productPriceValue = Double
										.parseDouble(productPrice.substring(0,
												productPrice.length() - 2));
							} catch (Exception e) {
						...
						Log.e("arraylst reltdprodts", arraylist + "");
						...
						// amountButton.setText(productPriceValue + "KD");//
						// Arun
						amountButton.setText(productPrice); // added
															// code
															// line

						// Product imageview view pager updation

						Log.e("ProdItemSingDetFrag", "The image list.."
								+ imageIdList);
						Log.e("ProdItemSingDetFrag", "ImageListCount : "
								+ imageIdList.size());
						...
						if (imageIdList.size() > 1) {
							...
							// indicator.setViewPager(productDetailsImageView);
						...
						Log.e("PromoItemSingProdDet", "Maximum Stock : "
								+ maxStock);
						if (maxStock > 10) {
							maxStock = 10;
						}
	...
	public void ShareDet() {
		Log.e("ShareDet",
				"ShareDet in PromoItemSingleProductDetailsFragment.java");
		Intent shareIntent = new Intent(Intent.ACTION_SEND);
		shareIntent.setType("text/plain");
